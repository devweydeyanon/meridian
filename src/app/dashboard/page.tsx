...
import { useDashboard, ACCOUNTS, CARDS, TRANSACTIONS, fmt, groupByDate } from './context';
...
export default function DashboardOverview() {
  ...
  const totalCredit = CARDS.filter(c => c.type === 'credit').reduce((s, c) => s + (c.balance || 0), 0);
  ...
  return (
    <>
      ...
      <div className="flex gap-2.5 mb-6 flex-wrap">
        ...
        <Link href="/dashboard/statements" className="flex items-center gap-2 px-4 py-2.5 text-[13px] font-semibold text-gray-700 bg-white border border-gray-200 rounded-lg no-underline hover:bg-gray-50 transition-all">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-4 h-4"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" /></svg>
          Statements
        </Link>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6 max-lg:grid-cols-2 max-md:grid-cols-1">
        {[
          ...
          { label: 'Investments & CDs', value: bal(totalCD), sub: '1 Certificate of Deposit' },
          { label: 'Credit Card Debt', value: bal(totalCredit), sub: '2 active cards', negative: true },
      ...
      {/* Account Cards */}
      <div className="grid grid-cols-2 gap-4 mb-6 max-lg:grid-cols-2 max-md:grid-cols-1">
        {ACCOUNTS.map(acc => (
          <Link key={acc.id} href="/dashboard/accounts" className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md hover:border-accent-500/30 transition-all no-underline cursor-pointer">
            <div className="flex justify-between items-start">
              <div>
                ...
                <div className="text-xs text-gray-400 mt-0.5">{acc.number}</div>
              </div>
          ...
          <div key={date}>
            ...
            {txns.map(txn => (
              <div key={txn.id} className="flex items-center px-5 py-3 border-b border-gray-50 hover:bg-gray-50/50 transition-all">
                ...
                {txn.status === 'pending' && (
                  <span className="ml-2 text-[10px] font-semibold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">Pending</span>
                )}
              </div>
            ))}
          </div>
